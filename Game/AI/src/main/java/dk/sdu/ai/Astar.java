...
public class Astar {
  ...
  private float heuristic(Node node, Node goal){
      ...
      float distance = (float) Math.sqrt(Math.pow((goalX-nodeX),2) + Math.pow( goalY-nodeY,2));
  ...
  private float cost(Node node){
      float pCost = node.getPath().size() * this.cost;
  ...
  for (Entity player : world.getEntities()){
      ...
      if(player.hasPart(SimpleMovingPart.class)){
          continue;
      }
 ...
 private ArrayList<Node> expand(World world, Entity entity, Node node){
  ...
  for (float[] element : sucessors){
    ...
    if(nodeChecker(neighbor, world, entity) != true) {
